...
{  atpBD.WorkMode:=false;
  ...
  Label4.Caption:='Генератор подключен непосредственно'+#13+
                  'к компьютеру, либо генератор и компьютер'+#13+
                  'включены в одну подсеть.';
  Label5.Caption:='Генератор и компьютер находятся'+#13+
                  'в разных подсетях.';
  Label7.Caption:='Эта программа предназначена для настройки'+#13+
                  'протокола TCP/IP в Г-420.'+#13+#13+
                  '1. Подключите генератор к компьютеру при'+#13+
                  '    помощи кабеля "PathCord" (кросс), либо'+#13+
                  '    соедините его с коммутатором нужной '+#13+
                  '    подсети.'+#13+
                  '2. Включите питание генератора.'+#13+
...
begin
  ...
  Label2.Caption:='Генератор зав.номер № '+
